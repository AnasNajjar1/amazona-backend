...
productRouter.get('/', expressAsyncHandler(async (req, res) => {
    ...
    const products = await Product.find({ ...sellerFilter, ...nameFilter, ...categoryFilter, ...priceFilter, ...ratingFilter }).populate('seller', 'seller.name seller.logo').populate('category', 'category.name').sort(sortOrder).skip(pageSize * (page - 1)).limit(pageSize);
