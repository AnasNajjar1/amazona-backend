...
productRouter.get('/', expressAsyncHandler(async (req, res) => {
    ...
    const products = await Product.find({ ...sellerFilter, ...nameFilter, ...categoryFilter, ...priceFilter, ...ratingFilter }).populate('seller', 'name seller.logo seller.rating seller.numReviews').sort(sortOrder).skip(pageSize * (page - 1)).limit(pageSize);
